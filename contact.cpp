...
public:
    ...
    void create_contact(string first, string last , string num ,string em) // Create node and add it onto the linked list
    {
        node *temp = new node;
        temp ->firstName = first;
        temp ->lastName = last;
        temp ->phoneNumber = num ;
        temp ->contactEmail = em ;
        temp->next = NULL;
        if (head == NULL)
        {
            head = temp;
    ...
    void display_all() // Prints out current trip
    {
        ...
        while (temp != NULL)    // Loop through the list while the temporary node is not empty
        {
            ...
            cout<<"Contact Email : "<<temp->contactEmail<<endl ;
            ++contactNum;
            temp = temp->next;
        }
    }
...
int main()
{
    ...
    while (1) {
        ...
        case 2:
            ...
            cout<<"\nEnter Email : ";
            cin>>email ;
            Contacts.create_contact(firstName, lastName,phoneNum ,email); // create the contact in the linked list
            cout << endl;
            break;
        case 3:
